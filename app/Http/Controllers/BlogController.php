...
class BlogController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware('permission:blog-write', ['only' => ['create', 'store', 'edit', 'update']]);
    }
    ...
    public function index(Request $request)
    {
        ...
        $query = DB::table('blogs')
            ->select('b_id', 'title', 'date', 'enabled')
            ->orderBy('date', 'desc');
    ...
    {
        $blog = DB::table('blogs')
            ->select('b_id', 'title', 'date', 'description', 'image_file', 'enabled')
            ->where('b_id', $b_id)
            ->first();
        ...
        return view('blogs.show', compact('blog', 'subImages'));
    }
    ...
    public function edit($b_id)
    {
        ...
        return view('blogs.edit', compact('blog', 'subImages'));
    }
    ...
    {
        return $this->handleRequest($request, $b_id);
    }

    // Helper methods
    protected function handleRequest(Request $request, $b_id = null)
    {
        $validator = $this->validateRequest($request, $b_id);
        ...
        try {
            ...
            if ($request->has('mainImage')) {
                if ($b_id) {
                    $oldMainImage = DB::table('blogs')->where('b_id', $b_id)->value('image_file');
                    if ($oldMainImage) {
                        Storage::disk('public')->delete($oldMainImage);
                    }
                }
                $mainImagePath = $request->file('mainImage')->store('blogs', 'public');
            ...
            if ($b_id) {
                DB::table('blogs')->where('b_id', $b_id)->update($data);
            } else {
                $data['created_by'] = auth()->id();
                $data['created_at'] = now();
                $b_id = DB::table('blogs')->insertGetId($data);
            }

            if ($request->hasFile('sub_images')) {
                $storedImagePaths = array_merge(
                    $storedImagePaths,
                    $this->storeSubImages($b_id, $request->file('sub_images'), $b_id ? true : false)
                );
            }
    ...
    protected function validateRequest(Request $request, $b_id = null)
    {
        ...
        return Validator::make($request->all(), [
            ...
            'mainImage' => ($b_id ? 'sometimes' : 'required').'|image|mimes:jpeg,png,jpg,gif,svg|max:102400',
    ...
    protected function storeSubImages($b_id, $subImages, $update = false)
    {
        $storedImagePaths = [];
        $imageData = [];
        foreach ($subImages as $subImage) {
            $storedImagePath = $subImage->store('blogs', 'public');
