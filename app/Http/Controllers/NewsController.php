...
class NewsController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = DB::table('news')
            ->select('n_id', 'title', 'date', 'enabled')
            ->orderBy('date', 'desc');
    ...
    {
        $news = DB::table('news')
            ->select('n_id', 'title', 'date', 'description', 'image_file', 'enabled')
            ->where('n_id', $n_id)
            ->first();

        $subImages = DB::table('news_images')
            ->select('ni_id', 'image_file')
            ->where('n_id', $n_id)
            ->get()
            ->toArray();

        return view('news.show', compact('news', 'subImages'));
    }
    ...
    public function edit($n_id)
    {
        ...
        return view('news.edit', compact('news', 'subImages'));
    }
    ...
    protected function handleRequest(Request $request, $n_id = null)
    {
        ...
        try {
            ...
            if ($request->has('mainImage')) {
                if ($n_id) {
                    $oldMainImage = DB::table('news')->where('n_id', $n_id)->value('image_file');
                    if ($oldMainImage) {
                        Storage::disk('public')->delete($oldMainImage);
                    }
                }
                $mainImagePath = $request->file('mainImage')->store('news', 'public');
            ...
            if ($n_id) {
                DB::table('news')->where('n_id', $n_id)->update($data);
            } else {
                $data['created_by'] = auth()->id();
                $data['created_at'] = now();
                $n_id = DB::table('news')->insertGetId($data);
            }
            ...
            if ($request->has('subImagesToDelete')) {
                ...
                DB::table('news_images')->whereIn('ni_id', $sub_image_ids)->delete();
            }

            $message = $n_id ? 'News updated successfully.' : 'News created successfully.';
    ...
    protected function storeSubImages($n_id, $subImages, $update = false)
    {
        ...
        foreach ($subImages as $subImage) {
            $storedImagePath = $subImage->store('news', 'public');
        ...
        DB::table('news_images')->insert($imageData);
