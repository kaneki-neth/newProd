...
@section('content')
        ...
        <div class="panel-body" id="pannel-body">
            <div class="table-responsive" style="overflow-x: hidden">
                <div class="d-flex justify-content-end">
                    <button class="btn btn-primary btn-xs" onclick="add_amenity_item()">
                    ...
                    <table id="data-table-scroller" width="100%"
                        ...
                        <thead>
                            <tr>
                                ...
                                <th class="text-center" style="width: 50%">Quantity</th>
                                <th class="text-center" style="width: 50%">Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($amenity_items as $amenity_item)
                                <tr>
                                    <td class="text-center">{{ $loop->iteration }}</td>
                                    <td id="name" style="color:#28acb5; cursor:pointer;"
                                        onclick="showmodaledit(`{{ $amenity_item->ai_id }}`)">
                                        {{ $amenity_item->name }}
                                    </td>
                                    <td class="text-center">{{ $amenity_item->description }}</td>
                                    <td class="text-center">{{ $amenity_item->price }}</td>
                                    <td class="text-center">{{ $amenity_item->quantity }}</td>
                                    <td class="text-center">
                                        @if($amenity_item->enabled == 1)
    ...
    <script>
        ...
        function add_amenity_item() {
            $.ajax({
                method: 'get',
                url: '{{ route("amenity_items.create") }}',
                contentType: false,
                processData: false,
                success: (response) => {
                    $("#main_modal").modal('show');
                    $("#modal-title").html('Add Amenity Item');
        ...
        function showmodaledit(id) {
            let url = '{{ route("amenity_items.edit", ["id" => ":id"]) }}';
