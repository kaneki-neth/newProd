...
<h1 class="page-header">Blog (Update)</h1>

<div class="panel panel-inverse">
    <div class="panel-body" id="pannel-body">
        <div class="row">
            <div class="col-md-12 d-flex justify-content-start gap-2">
                <a href="/blogs/{{ $blog->b_id }}" class="btn btn-primary btn-xs"><i class="fa fa-eye"></i> View</a>
            </div>
        </div>
        <div class="row mt-3">
        ...
        <div class="mt-3 d-flex flex-row gap-2">
            ...
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" role="switch" id="enabled" name="enabled" {{ $blog->enabled ? 'checked' : '' }}>
            </div>
        </div>
...
<script> 
    ...
    $(document).ready(function() {
        ...
        $('#mainImagePreview').attr('src', '/storage/' + blog.image_file + '?t=' + new Date().getTime());
        ...
        if(subImages) {
            subImages.forEach(sub_image => {
                ...
                toolOverlay.addEventListener('click', function() {
                    let bi_id = sub_image.bi_id;
                    console.log(bi_id);
                    deleteOldSubImage(newDiv, bi_id);
                });
    ...
    function submitData() {
        ...
        $.ajax({
            ...
            success: function (response) {
                window.location.href = '/blogs/' + {{ $blog->b_id }};
            },
