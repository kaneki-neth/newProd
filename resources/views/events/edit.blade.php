...
<h1 class="page-header">Event (Update)</h1>

<div class="panel panel-inverse">
    <div class="panel-body" id="pannel-body">
        <div class="row">
            <div class="col-md-12 d-flex justify-content-start gap-2">
                <a href="/events_/{{ $event->e_id }}" class="btn btn-primary btn-xs"><i class="fa fa-eye"></i> View</a>
            </div>
        </div>
        <div class="row mt-3">
        ...
        <div class="mt-3 d-flex flex-row gap-2">
            ...
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" role="switch" id="enabled" name="enabled" {{ $event->enabled ? 'checked' : '' }}>
            </div>
        </div>
...
<script> 
    ...
    $(document).ready(function() {
        ...
        $.getScript("/assets/plugins/summernote/dist/summernote-lite.min.js", function() {
            ...
            $('#summernote').summernote('code', event.description);
        });
        ...
        if(subImages) {
            subImages.forEach(sub_image => {
                ...
                toolOverlay.addEventListener('click', function() {
                    let ei_id = sub_image.ei_id;
                    console.log(ei_id);
                    deleteOldSubImage(newDiv, ei_id);
                });
    ...
    function submitData() {
        ...
        let url = '/events_/' + {{ $event->e_id }} + '/edit';
        $.ajax({
            url: url,
            type: 'POST',
            data: formData,
            ...
            success: function (response) {
                window.location.href = '/events_/' + {{ $event->e_id }};
            },
