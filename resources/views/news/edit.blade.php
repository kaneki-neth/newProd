    ...
    <div class="panel-body" id="pannel-body">
        <div class="row">
            <div class="col-md-12 d-flex justify-content-start gap-2">
                <a href="/news/{{ $news->n_id }}" class="btn btn-primary btn-xs"><i class="fa fa-eye"></i> View</a>
            </div>
        </div>
        <div class="row mt-3">
...
<script> 
    ...
    $(document).ready(function() {
        ...
        $('#mainImagePreview').attr('src', '/storage/' + news.image_file + '?t=' + new Date().getTime());
        ...
        $.getScript("/assets/plugins/summernote/dist/summernote-lite.min.js", function() {
            ...
            $('#summernote').summernote('code', news.description);
        });
        ...
        if(subImages) {
            subImages.forEach(sub_image => {
                ...
                toolOverlay.addEventListener('click', function() {
                    let ni_id = sub_image.ni_id;
                    console.log(ni_id);
                    deleteOldSubImage(newDiv, ni_id);
                });
    ...
    function submitData() {
        ...
        $.ajax({
            ...
            success: function (response) {
                window.location.href = '/news/' + {{ $news->n_id }};
            },
